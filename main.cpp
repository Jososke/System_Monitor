...
void writeSysInfoToConsole(SysInfo sys, WINDOW* sys_win){
    ...
    //mvwprintw(sys_win,4,2,getCString(("CPU: " + sys.getCpuPercent())));
...
void getProcessListToConsole(std::vector<string> processes,WINDOW* win){
    ...
    mvwprintw(win,1,7,"User:"); //We just changed some values so that we get nice UI
    mvwprintw(win,1,17,"CPU[%]:"); //For this go through the videos of ncurses library
    mvwprintw(win,1,27,"RAM [MB]:");
    mvwprintw(win,1,37,"Uptime:");
    mvwprintw(win,1,47,"CMD:");
